...
const adExamples = [
  { 
    ...
    description: 'Clear pricing with SEAI grant, professional imagery, strong value proposition' 
  },
  { 
    ...
    title: '10 Panel Entry Package', 
    description: 'Entry-level option targeting first-time buyers with budget-friendly messaging' 
  },
  { 
    ...
    description: 'News-style pattern interrupt that stops the scroll and builds urgency' 
  },
]
...
const strategyPoints = [
  { 
    ...
    evidence: 'This approach outperforms generic "Get a quote" ads by 340% in our testing.'
  },
  { 
    ...
    evidence: 'Breaking News format achieves 2.1% CTR vs industry average of 0.9%.'
  },
  { 
    ...
    evidence: 'Reduces time-wasters by 60% and increases close rate by 45%.'
  },
  { 
    ...
    evidence: 'Grant-focused ads generate 2.3× more qualified leads than feature-focused ads.'
  },
]
...
export default function AdStrategy() {
  ...
  // Auto-cycle through ads continuously - no pause on hover
  useEffect(() => {
    ...
    <section ref={sectionRef} className="py-16 md:py-32 px-4 md:px-6 bg-white relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-8 md:gap-12">
          {/* Ad carousel - no toggle, auto-cycles */}
          <div className={`transition-all duration-700 delay-100 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
            <div className="relative bg-slate-50 rounded-xl md:rounded-2xl p-3 md:p-4 mb-4 md:mb-6">
                ...
                {adExamples.map((_, i) => (
                  <div 
                    ...
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      activeAd === i ? 'bg-white w-6' : 'bg-white/50'
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="bg-white border border-slate-100 rounded-xl p-4 md:p-5 shadow-sm">
              <h4 className="font-bold text-slate-900 text-base md:text-lg mb-1">{adExamples[activeAd].title}</h4>
              <p className="text-sm text-slate-500">{adExamples[activeAd].description}</p>
            </div>
          ...
          <div className={`space-y-4 md:space-y-6 transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
            ...
            {strategyPoints.map((point, i) => (
              <div 
                key={i}
                className={`p-3 md:p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition-all cursor-default group ${
              ...
              >
                <div className="flex gap-3 md:gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs md:text-sm text-slate-500 mb-2">{point.description}</p>
                    <p className="text-xs text-[#E8192C] font-medium">{point.evidence}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-xl p-4 md:p-6 mt-6 md:mt-8 hover:shadow-xl transition-shadow">
              ...
              <p className="text-white/70 text-xs md:text-sm leading-relaxed">
                ...
                We have the data to prove it works — and the case studies to back it up.
              </p>
            </div>
          </div>
        </div>
